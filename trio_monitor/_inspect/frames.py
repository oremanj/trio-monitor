...
def inspect_frame(frame: FrameType) -> FrameDetails:
    ...
    if sys.implementation.name == "cpython":
        from .frame_tricks_cpython import inspect_frame
    elif sys.implementation.name == "pypy":
        from .frame_tricks_pypy import inspect_frame
    else:
        raise NotImplementedError("frame details not supported on this interpreter")
    return inspect_frame(frame)


def analyze_with_blocks(code: CodeType) -> Dict[int, ContextInfo]:
...
def currently_exiting_context(frame: FrameType) -> Optional[ExitingContext]:
